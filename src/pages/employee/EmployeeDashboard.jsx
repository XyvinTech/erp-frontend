...
function EmployeeDashboard() {
  ...
  const fetchAttendanceStats = async () => {
    try {
      ...
      const response = await attendanceService.get('/attendance/my-attendance', {
        params: {
  ...
  const fetchRecentPayslips = async () => {
    try {
      ...
      const response = await payrollService.get('/my-payroll');
      ...
      const sortedPayroll = payrollData
        .sort((a, b) => new Date(a.period || a.date) - new Date(b.period || b.date))
        .slice(-6);
      ...
      const formattedPayslips = sortedPayroll.map(payroll => ({
        ...
        deductions: parseFloat(payroll.deductions || payroll.totalDeductions) || 0
      }));
  ...
  const renderStatisticCards = () => (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
      <Card className="p-4">
        ...
        <p className="mt-2 text-3xl font-semibold text-primary-600">{attendanceStats.percentage}%</p>
        <p className="mt-1 text-sm text-gray-600">{attendanceStats.present} days present</p>
      </Card>
      <Card className="p-4">
        ...
        <p className="mt-1 text-sm text-gray-600">Annual leave days remaining</p>
      </Card>
      <Card className="p-4">
        <h3 className="text-sm font-medium text-gray-500">Late Arrivals</h3>
        <p className="mt-2 text-3xl font-semibold text-primary-600">{attendanceStats.late}</p>
  ...
  const renderAttendanceChart = () => {
    ...
    return (
      <Card className="p-6">
        <h2 className="text-lg font-semibold mb-4">Monthly Attendance Distribution</h2>
        <div className="h-80">
        ...
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="used" stroke="#8884d8" name="Used Days" />
              <Line type="monotone" dataKey="remaining" stroke="#82ca9d" name="Remaining Days" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </Card>
  ...
  const renderPayrollChart = () => {
    const chartData = recentPayslips.length > 0 ? recentPayslips : generateDefaultPayslips();
