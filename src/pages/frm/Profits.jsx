...
const Profits = () => {
  ...
  const [deleteModal, setDeleteModal] = useState({ isOpen: false, profit: null });
  ...
  const handleSubmit = async (data) => {
    try {
      ...
      toast.error(error.response?.data?.message || error.message || 'Failed to save profit');
    }
  };
  ...
  const confirmDelete = async () => {
    try {
      ...
      toast.error(error.response?.data?.message || error.message || 'Failed to delete profit');
    } finally {
        ...
        Cell: ({ value }) => (
          <span
            className={`inline-flex rounded-full px-2 text-xs font-semibold leading-5 ${
              value === 'Realized' ? 'bg-green-100 text-green-800' :
